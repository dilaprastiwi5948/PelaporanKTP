...
class DailyReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($date) {
            ...
            $data->explanation = ExplanationType::select(['explanation_types.name', DB::raw('(select count(id) total from reporting_id_cards where explanationtype_id = explanation_types.id AND date(created_at) = "'.$date.'" group by explanationtype_id) total')])->get();
            $data->submission = SubmissionType::select(['submission_types.name', DB::raw('(select count(id) total from reporting_id_cards where submissiontype_id = submission_types.id AND date(created_at) = "'.$date.'" group by submissiontype_id) total')])->get();
            // $data->category = [
    ...
    public function print(Request $req)
    {
        ...
        if ($date) {
            ...
            $pdf = PDF::setOption(['isHtml5ParserEnabled' => true])->loadView($this->viewFolder. 'print', [
                'data' => $data,
            ])->setPaper('a4', 'landscape');
            return $pdf->stream();
            // return $pdf->download("Laporan ".date('d F Y', strtotime($date)).".pdf");
        }
        return redirect()->route('report.dailyreport');
    }
}
